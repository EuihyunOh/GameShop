function go_save() {
  if (document.formm.id.value == "") {
    ...
    document.formm.action = "gameshopServlet?command=join";
...
function idcheck() {
  ...
  var url = "gameshopServlet?command=id_check_form&id=" 
+ document.formm.id.value;
  window.open( url, "_blank_1", "toolbar=no, menubar=no, scrollbars=no, resizable=no, width=330, height=200");
}

function post_zip() {
  var url = "gameshopServlet?command=find_zip_num";
  window.open( url, "_blank_1", "toolbar=no, menubar=no, scrollbars=yes, resizable=no, width=550, height=300, top=300, left=300, ");
}

function go_next() {
  if (document.formm.okon1[0].checked == true) {
    document.formm.action = "gameshopServlet?command=join_form";
    document.formm.submit();
